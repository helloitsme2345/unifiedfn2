...
// CUDA/C/C++ source code generated on  : 07-Sep-2020 10:22:56
//
  ...
  double T, double op[8])
{
  ...
  cudaMallocManaged(&gpu_C, 832ULL);
  cudaMallocManaged(&b_gpu_y_meas, 104ULL);
  cudaMallocManaged(&gpu_A, 832ULL);
  cudaMallocManaged(&gpu_hk, 104ULL);
  cudaMallocManaged(&gpu_H_linear, 832ULL);
  cudaMallocManaged(&b_gpu_Re_inv, 1352ULL);
  cudaMallocManaged(&gpu_Re_inv, 104ULL);
  cudaMallocManaged(&gpu_ik, 64ULL);
  cudaMallocManaged(&gpu_ih, 64ULL);
  cudaMallocManaged(&gpu_calcEq, 512ULL);
  cudaMallocManaged(&c_gpu_Ih, 256ULL);
  cudaMallocManaged(&b_gpu_Ih, 256ULL);
  cudaMallocManaged(&b_gpu_x, 128ULL);
  cudaMallocManaged(&b_gpu_Gk, 256ULL);
  cudaMallocManaged(&gpu_Ih, 512ULL);
  cudaMallocManaged(&gpu_Gk, 256ULL);
  cudaMallocManaged(&b_gpu_p, 8ULL);
  cudaMallocManaged(&b_gpu_ipiv, 8ULL);
  cudaMallocManaged(&gpu_x, 512ULL);
  cudaMallocManaged(&gpu_Fk_inv, 512ULL);
  cudaMallocManaged(&gpu_p, 4ULL);
  cudaMallocManaged(&gpu_ipiv, 4ULL);
  cudaMallocManaged(&gpu_Rw_inv, 128ULL);
  cudaMallocManaged(&gpu_Rw, 128ULL);
  cudaMallocManaged(&gpu_op, 64ULL);
  cudaMallocManaged(&b_gpu_Rw, 32ULL);
  cudaMallocManaged(&gpu_iv, 8ULL);
  cudaMallocManaged(&gpu_iv1, 8ULL);
  cudaMallocManaged(&gpu_iv2, 8ULL);
  cudaMallocManaged(&gpu_a, 512ULL);
  cudaMallocManaged(&gpu_B_usedMeas_vec, 104ULL);
  cudaMallocManaged(&gpu_Re, 104ULL);
  cudaMallocManaged(&gpu_iv3, 8ULL);
  cudaMallocManaged(&gpu_iv4, 8ULL);
  cudaMallocManaged(&gpu_iv5, 8ULL);
  cudaMallocManaged(&gpu_y_meas, 104ULL);
  cudaMemcpy(gpu_y_meas, (void *)&y_meas[0], 104ULL, cudaMemcpyHostToDevice);
  cudaMemcpy(gpu_iv5, (void *)&iv5[0], 8ULL, cudaMemcpyHostToDevice);
  cudaMemcpy(gpu_iv4, (void *)&iv4[0], 8ULL, cudaMemcpyHostToDevice);
  cudaMemcpy(gpu_iv3, (void *)&iv3[0], 8ULL, cudaMemcpyHostToDevice);
  cudaMemcpy(gpu_Re, (void *)&Re[0], 104ULL, cudaMemcpyHostToDevice);
  cudaMemcpy(gpu_B_usedMeas_vec, (void *)&B_usedMeas_vec[0], 104ULL,
             cudaMemcpyHostToDevice);
  cudaMemcpy(gpu_a, (void *)&a[0], 512ULL, cudaMemcpyHostToDevice);
  cudaMemcpy(gpu_iv2, (void *)&iv2[0], 8ULL, cudaMemcpyHostToDevice);
  cudaMemcpy(gpu_iv1, (void *)&iv1[0], 8ULL, cudaMemcpyHostToDevice);
  cudaMemcpy(gpu_iv, (void *)&iv[0], 8ULL, cudaMemcpyHostToDevice);
  cudaMemcpy(b_gpu_Rw, (void *)&Rw[0], 32ULL, cudaMemcpyHostToDevice);
  ...
  cudaDeviceSynchronize();
  cudaMemcpy(&op[0], gpu_op, 64ULL, cudaMemcpyDeviceToHost);
  cudaFree(*gpu_y_meas);
  cudaFree(*gpu_iv5);
  cudaFree(*gpu_iv4);
  cudaFree(*gpu_iv3);
